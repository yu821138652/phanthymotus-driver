# Unitree Go1 · go1_bundle（只读状态驱动 · 卡片开发蓝本）
...
- 所有卡共用**同一个只读** raw SDK client（`go1_sdk_client.py`）：唯一的 UDP 收发线程，把 `HighState` 解析成一份线程安全的 `snapshot()`。各卡只是 `snapshot()` 的不同切片。
...
```
go1_bundle/
├── main.py            # MCP server 入口 + 按 config 卡名自动装配（HIGHLEVEL 只读）
